...
In the other hand, to manage the active tab item and the movement of the active
mark, we have to use the `UseActiveTab` hook [(More info)](#useactivetab) and
integrate what it provides with our components.
...
its children will inherit them automatically, as in this example:
