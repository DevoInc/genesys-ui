...
import { SOURCE_CODE_INTERACTIVE_WITH_CHILDREN } from './__stories__';
...
If you create a case of the MenuItem, but using the children instead of the content props,
and you have interactive content, then you have to use the `Menu.Item._Wrapper` and the
`Menu.Item._Inner` to create your content. After that you have to include your interactive content
after the 'inner' and inside the 'wrapper' block.

In addition, you can use the `useMenuItemPadding` to apply the necessary padding-right to the MenuItem
based in the width of your custom interactive block. You only have to pass as hook arguments the reference of
your MenuItem and your interactive block.

<Source code={SOURCE_CODE_INTERACTIVE_WITH_CHILDREN} />

<Canvas of={Stories.WithInteractiveContentAndChildren} />
