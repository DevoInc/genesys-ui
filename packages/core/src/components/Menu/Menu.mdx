...
A `Menu` generally represents a grouping of common actions or functions that the
user can invoke. It can be also used to create navigation menu based in links,
so in this case it's better to use the prop `cmpRole` to `nav`.
...
value. In addition, a checkbox or radio is rendered inside the menu item, but
it's not visible, its purpose is manage the selection behavior.
