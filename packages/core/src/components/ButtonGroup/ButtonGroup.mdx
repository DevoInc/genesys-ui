...
- **ButtonGroup.Divider**: it's the equivalent to the `Divider` component, but
  it has a default height with the same value as the buttons in the group.
...
We can use the `ButtonGroup.Divider` component to separate buttons.
