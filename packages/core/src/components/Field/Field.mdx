...
and validation messages. It also manages the layout and distribution of these
elements within the field control.
...
This component is usually used internally by the form components as `Input`, `Select`, `Checkbox`... etc.
so they are a composition of a `Field` component as parent with the field control as child (e.g. `InputControl`).
Therefore, is not usual to use it, at least you need to create a new form field.

The component is used as a wrapper of an element (usually field controls). We can get any case of the component
using only `Field` and its props as in this example with an `InputControl` as child.
...
## Required, label and helper

This example provides a required marker, helper and label to the field control.

<Canvas sourceState="shown" of={Stories.LabelAndHelper} />
...
- [SelectControl](?path=/docs/components-form-selectcontrol--overview)
- [SwitchControl](?path=/docs/components-form-switchcontrol--overview)
- [TextareaControl](?path=/docs/components-form-textareacontrol--overview)
